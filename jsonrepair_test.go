...
func Test_RepairJSON(t *testing.T) {
    ...
    ",п"г`,
		...
		{
			in:   `{"result":"摘要包含"目的""方法""结果""结论"四要素。","passed":false}`,
			want: `{"result":"摘要包含\"目的\"\"方法\"\"结果\"\"结论\"四要素。","passed":false}`,
		},
		{
			in:   `{"result":"结语以"结论"为标题，命名规范，但存在问题：仅笼统提及"复杂工程特性"，建议修改为"客观表述"。","passed":false}`,
			want: `{"result":"结语以\"结论\"为标题,命名规范,但存在问题:仅笼统提及\"复杂工程特性\",建议修改为\"客观表述\"。","passed":false}`,
		},
		{
			in:   `{"dimension_id":38,"result":"监测频率数据矛盾——3.3节"每半天开展两次"与前后文逻辑跳跃，需核实；参考文献[1][2][3]标注发表时间为2025年且期号分别为"08""16""13"，需核实；变量下标"x_x"应为"x_n"，属明显笔误。"}`,
			want: `{"dimension_id":38,"result":"监测频率数据矛盾——3.3节\"每半天开展两次\"与前后文逻辑跳跃,需核实;参考文献[1][2][3]标注发表时间为2025年且期号分别为\"08\"\"16\"\"13\",需核实;变量下标\"x_x\"应为\"x_n\",属明显笔误。"}`,
		},
		{
			in:   `{"姓名":"张三","结论":"通过"}`,
			want: `{"姓名":"张三","结论":"通过"}`,
